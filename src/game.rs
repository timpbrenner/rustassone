...
pub fn create_game() -> JsGame {
    ...
    play_tile(game.id, TilePlay { tile_id: Some(4), player_id: Some(-1), row_offset: Some(0), column_offset: Some(0) });
...
fn build_game_grid(current_game_id: i32, connection: PgConnection) -> Vec<Vec<JsTile>> {
    ...
    let max_column_offset = played_tiles.iter().max_by_key(|x| println!("COLUMN: {}", x.column_offset)).unwrap().column_offset + 1;
